...
// --------------------------------------------------------------------------
CosNaming::NamingContext_ptr getContext(const string &cname, int argc,
...
// --------------------------------------------------------------------------
CosNaming::NamingContext_ptr getContext(const CORBA::ORB_ptr orb,
                                        const string &cname,
                                        const string &servname) {
  CosNaming::NamingContext_var rootC;

  ulogrep << "OREPHELPER(getContext): get rootcontext...(servname = "
          << servname << ")" << endl;
  ...
  if (CORBA::is_nil(rootC)) {
    ...
    throw ORepFailed(
        "OREPHELPER(getContext): не смог получить ссылку на NameServices");
  }
  ...
  try {
    ...
    if (CORBA::is_nil(ctx)) {
      const string err("OREPHELPER(getContext): не смог получить ссылку на "
                       "контекст(is_nil) " +
                       cname);
    ...
    ostringstream err;
    err << "OREPHELPER(getContext): не смог получить ссылку на контекст "
        << cname;
    uwarn << err.str() << endl;
    ...
    ostringstream err;
    err << "OREPHELPER(getContext): поймали CORBA::SystemException: "
        << ex.NP_minorString();
    uwarn << err.str() << endl;
    throw ORepFailed(err.str());
...
CosNaming::NamingContext_ptr getRootNamingContext(const CORBA::ORB_ptr orb,
  ...
  try {
    //        cout << "ORepHelpers(getRootNamingContext): nsName->" << nsName <<
    //        endl;
    CORBA::Object_var initServ =
        orb->resolve_initial_references(nsName.c_str());
    ulogrep << "OREPHELP: get rootcontext...(nsName = " << nsName << ")"
            << endl;

    if (CORBA::is_nil(initServ)) {
      const string err("ORepHelpers: fail resolve_initial_references '" +
                       nsName + "'");
      throw ORepFailed(err.c_str());
    }
    ...
    if (CORBA::is_nil(rootContext)) {
      const string err(
          "ORepHelpers: Не удалось преобразовать ссылку к нужному типу.");
      throw ORepFailed(err.c_str());
    }
    ...
    ostringstream err;
    err << "ORepHelpers(getRootNamingContext): Не смог получить ссылку на "
           "контекст ->"
        << nsName;
    throw ORepFailed(err.str());
    ...
    const string err("ORepHelpers(getRootNamingContext): Caught a system "
                     "exception while resolving the naming service.");
    throw ORepFailed(err);
  }
...
// ---------------------------------------------------------------------------------------------------------------
/*!
 *    \param fname - полное имя включающее в себя путь
 * ("Root/Section1/name|Node:Alias") \param brk  - используемый символ
 * разделитель
 */
 ...
 *  \note Функция возвращает путь без последнего символа разделителя
 * ("Root/Section1/name" -> "Root/Section1")
 */

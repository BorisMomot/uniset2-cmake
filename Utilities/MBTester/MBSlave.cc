...
// -------------------------------------------------------------------------
MBSlave::MBSlave(const std::unordered_set<ModbusAddr> &_vaddr,
                 const std::string &dev, const std::string &speed, bool use485)
    : rscomm(NULL), vaddr(_vaddr),
      //    prev(ModbusRTU::erNoError),
      //    askCount(0),
      verbose(false), replyVal(-1), replyVal2(-1), replyVal3(-1) {
  ...
  rscomm->connectReadInputStatus(
      sigc::mem_fun(this, &MBSlave::readInputStatus));
  ...
  rscomm->connectForceSingleCoil(
      sigc::mem_fun(this, &MBSlave::forceSingleCoil));
  rscomm->connectForceCoils(sigc::mem_fun(this, &MBSlave::forceMultipleCoils));
  rscomm->connectWriteOutput(
      sigc::mem_fun(this, &MBSlave::writeOutputRegisters));
  rscomm->connectWriteSingleOutput(
      sigc::mem_fun(this, &MBSlave::writeOutputSingleRegister));
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::readInputStatus(ReadInputStatusMessage &query,
  ...
  if (rndgen) {
    ...
    while (i < query.count) {
      ...
      for (unsigned int nbit = 0; nbit < BitsPerByte && i < query.count;
           nbit++, i++)
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::readOutputRegisters(ModbusRTU::ReadOutputMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::forceMultipleCoils(ModbusRTU::ForceCoilsMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::writeOutputRegisters(ModbusRTU::WriteOutputMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode MBSlave::writeOutputSingleRegister(
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::forceSingleCoil(ModbusRTU::ForceSingleCoilMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::journalCommand(ModbusRTU::JournalCommandMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::setDateTime(ModbusRTU::SetDateTimeMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::remoteService(ModbusRTU::RemoteServiceMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::fileTransfer(ModbusRTU::FileTransferMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBSlave::diagnostics(ModbusRTU::DiagnosticMessage &query,
...
ModbusRTU::mbErrCode MBSlave::read4314(ModbusRTU::MEIMessageRDI &query,
  ...
  if (query.objID == rdiVendorName) {
    ...
    reply.mf = 0xFF;
    reply.conformity = rdevBasicDevice;
    reply.addData(query.objID, "unsiet2");
    return erNoError;
  } else if (query.objID == rdiMajorMinorRevision) {
    reply.mf = 0xFF;
    reply.conformity = rdevBasicDevice;
    reply.addData(query.objID,
                  "package version is not supported yet"); // TODO generate
                                                           // package version
    return erNoError;

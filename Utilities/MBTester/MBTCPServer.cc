...
#ifndef PACKAGE_URL
#define PACKAGE_URL                                                            \
  "http://git.etersoft.ru/projects/?p=asu/uniset.git;a=summary"
#endif
// -------------------------------------------------------------------------
using namespace std;
...
// -------------------------------------------------------------------------
MBTCPServer::MBTCPServer(const std::unordered_set<ModbusAddr> &myaddr,
                         const string &inetaddr, int port, bool verb)
    : sslot(NULL), vaddr(myaddr),
      //    prev(ModbusRTU::erNoError),
      //    askCount(0),
      verbose(verb), replyVal(-1) {
  //    int replyTimeout = uni_atoi(
  //    conf->getArgParam("--reply-timeout",it.getProp("reply_timeout")).c_str()
  //    ); if( replyTimeout <= 0 )
  //        replyTimeout = 2000;
  ...
  sslot->connectReadInputStatus(
      sigc::mem_fun(this, &MBTCPServer::readInputStatus));
  sslot->connectReadOutput(
      sigc::mem_fun(this, &MBTCPServer::readOutputRegisters));
  sslot->connectReadInput(
      sigc::mem_fun(this, &MBTCPServer::readInputRegisters));
  sslot->connectForceSingleCoil(
      sigc::mem_fun(this, &MBTCPServer::forceSingleCoil));
  sslot->connectForceCoils(
      sigc::mem_fun(this, &MBTCPServer::forceMultipleCoils));
  sslot->connectWriteOutput(
      sigc::mem_fun(this, &MBTCPServer::writeOutputRegisters));
  sslot->connectWriteSingleOutput(
      sigc::mem_fun(this, &MBTCPServer::writeOutputSingleRegister));
  ...
  sslot->connectJournalCommand(
      sigc::mem_fun(this, &MBTCPServer::journalCommand));
...
// -------------------------------------------------------------------------
MBTCPServer::~MBTCPServer() { delete sslot; }
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
void MBTCPServer::setMaxSessions(size_t max) { sslot->setMaxSessions(max); }
// -------------------------------------------------------------------------
void MBTCPServer::setRandomReply(long min, long max) {
  rndgen = make_unique<std::uniform_int_distribution<>>(min, max);
}
// -------------------------------------------------------------------------
void MBTCPServer::execute() { sslot->run(vaddr); }
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::readInputStatus(ReadInputStatusMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::readOutputRegisters(ModbusRTU::ReadOutputMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::forceMultipleCoils(ModbusRTU::ForceCoilsMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::writeOutputRegisters(ModbusRTU::WriteOutputMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode MBTCPServer::writeOutputSingleRegister(
    ModbusRTU::WriteSingleOutputMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::forceSingleCoil(ModbusRTU::ForceSingleCoilMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::journalCommand(ModbusRTU::JournalCommandMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::setDateTime(ModbusRTU::SetDateTimeMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::remoteService(ModbusRTU::RemoteServiceMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::fileTransfer(ModbusRTU::FileTransferMessage &query,
...
// -------------------------------------------------------------------------
ModbusRTU::mbErrCode
MBTCPServer::diagnostics(ModbusRTU::DiagnosticMessage &query,
  ...
  if (query.subf == ModbusRTU::dgMsgSlaveCount ||
      query.subf == ModbusRTU::dgBusMsgCount) {
    reply = query;
    reply.data[0] = 10;
    return ModbusRTU::erNoError;
  }
...
ModbusRTU::mbErrCode MBTCPServer::read4314(ModbusRTU::MEIMessageRDI &query,
  ...
  if (query.objID == rdiVendorName) {
    ...
    reply.mf = 0xFF;
    reply.conformity = rdevBasicDevice;
    reply.addData(query.objID, "uniset2");
    return erNoError;
    ...
    reply.addData(
        query.objID,
        "version is not supported yet"); // TODO add version generation
    return erNoError;

...
MBConfig::MBConfig(const std::shared_ptr<uniset::Configuration> &_conf,
                   xmlNode *_cnode, std::shared_ptr<SMInterface> _shm)
    : cnode(_cnode), conf(_conf), shm(_shm) {}
// -----------------------------------------------------------------------------
MBConfig::~MBConfig() {}
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
void MBConfig::loadConfig(const std::shared_ptr<uniset::UniXML> &xml,
...
// ------------------------------------------------------------------------------------------
void MBConfig::readConfiguration(const std::shared_ptr<uniset::UniXML> &xml,
                                 UniXML::iterator sensorsSection) {
  ...
  for (; it.getCurrent(); it.goNext()) {
    if (uniset::check_filter(it, s_field, s_fvalue) && !initItem(it)) {
      ostringstream err;
      err << myname
          << "(readConfiguration): Error during sensor configuration name -> "
          << it.getProp("name");
      throw SystemError(err.str());
    }
  }
...
// ------------------------------------------------------------------------------------------
MBConfig::DeviceType
MBConfig::getDeviceType(const std::string &dtype) noexcept {
  if (dtype.empty())
    return dtUnknown;
...
std::ostream &operator<<(std::ostream &os, MBConfig::RTUDevice &d) {
  os << "addr=" << ModbusRTU::addr2str(d.mbaddr) << " type=" << d.dtype
     << " respond_id=" << d.resp_id
     << " respond_timeout=" << d.resp_Delay.getOffDelay()
     << " respond_state=" << d.resp_state << " respond_invert=" << d.resp_invert
     << " safeMode=" << (MBConfig::SafeMode)d.safeMode << endl;
...
// -----------------------------------------------------------------------------
std::ostream &operator<<(std::ostream &os, const MBConfig::RegInfo &r) {
  os << " id=" << r.regID << " mbreg=" << ModbusRTU::dat2str(r.mbreg)
     << " mbfunc=" << r.mbfunc << " q_num=" << r.q_num
     << " q_count=" << r.q_count << " value=" << ModbusRTU::dat2str(r.mbval)
     << "(" << (int)r.mbval << ")"
     << " mtrType=" << MTR::type2str(r.mtrType) << endl;
...
// -----------------------------------------------------------------------------
void MBConfig::rtuQueryOptimizationForDevice(
    const std::shared_ptr<RTUDevice> &d, size_t maxQueryCount) {
  //      mbinfo << myname << "(rtuQueryOptimizationForDevice): dev addr="
  //             << ModbusRTU::addr2str(d->mbaddr) << " optimization..." <<
  //             endl;
...
// -----------------------------------------------------------------------------
void MBConfig::rtuQueryOptimizationForRegMap(
    const std::shared_ptr<RegMap> &regmap, size_t maxQueryCount) {
  if (regmap->size() <= 1)
    return;

  // Вообще в map они уже лежат в нужном порядке, т.е. функция genRegID()
  // гарантирует что регистры идущие подряд с одниковой функцией чтения/записи
  // получат подряд идущие RegID. так что оптимтизация это просто нахождение
  // мест где RegID идут не подряд...
  for (auto it = regmap->begin(); it != regmap->end(); ++it) {
    ...
    if (beg->q_count > 1 &&
        beg->mbfunc == ModbusRTU::fnWriteOutputSingleRegister) {
      //              mbwarn << myname << "(rtuQueryOptimization): "
      //                     << " optimization change func=" <<
      //                     ModbusRTU::fnWriteOutputSingleRegister
      //                     << " <--> func=" <<
      //                     ModbusRTU::fnWriteOutputRegisters
      //                     << " for mbaddr=" <<
      //                     ModbusRTU::addr2str(beg->dev->mbaddr)
      //                     << " mbreg=" << ModbusRTU::dat2str(beg->mbreg);

      beg->mbfunc = ModbusRTU::fnWriteOutputRegisters;
    } else if (beg->q_count > 1 &&
               beg->mbfunc == ModbusRTU::fnForceSingleCoil) {
      //              mbwarn << myname << "(rtuQueryOptimization): "
      //                     << " optimization change func=" <<
      //                     ModbusRTU::fnForceSingleCoil
...
// -----------------------------------------------------------------------------
std::shared_ptr<MBConfig::RTUDevice> MBConfig::addDev(RTUDeviceMap &mp,
                                                      ModbusRTU::ModbusAddr a,
                                                      UniXML::iterator &xmlit) {
  auto it = mp.find(a);
  ...
  if (it != mp.end()) {
    ...
    if (it->second->dtype != dtype) {
      mbcrit << myname << "(addDev): OTHER mbtype=" << dtype << " for "
             << xmlit.getProp("name") << " prop='" << prop_prefix + "mbtype"
             << "'"
...
// ------------------------------------------------------------------------------------------
std::shared_ptr<MBConfig::RegInfo>
MBConfig::addReg(std::shared_ptr<RegMap> &mp, ModbusRTU::RegID regID,
                 ModbusRTU::ModbusData r, UniXML::iterator &xmlit,
                 std::shared_ptr<MBConfig::RTUDevice> dev) {
  ...
  if (it != mp->end()) {
    ...
    if (it->second->dev->dtype != dev->dtype) {
      mbcrit << myname << "(addReg): OTHER mbtype=" << dev->dtype << " for "
             << xmlit.getProp("name")
             << ". Already used devtype=" << it->second->dev->dtype << " for "
             << it->second->dev << endl;
      return nullptr;
    }
    ...
    mbinfo << myname << "(addReg): reg=" << ModbusRTU::dat2str(r)
           ...
           << " Ignore register params for " << xmlit.getProp("name") << " ..."
           << endl;
  ...
  mbinfo << myname << "(addReg): reg=" << ModbusRTU::dat2str(r)
         << "(regID=" << regID << ") mapsize=" << mp->size() << endl;
...
bool MBConfig::initRSProperty(RSProperty &p, UniXML::iterator &it) {
  ...
  if (p.nbit > 0 &&
      (p.stype == UniversalIO::AI || p.stype == UniversalIO::AO)) {
    mbwarn << "(initRSProperty): (ignore) incorrect param`s nbit!=0(" << p.nbit
           << ")"
           << " for iotype=" << p.stype << " for " << it.getProp("name")
           << endl;
  }
  ...
  if (!smask.empty()) {
    if (p.nbit != -1) {
      mbcrit << myname
             << "(initRSProperty): config error: nbit!=0 and mbmask!=0 for "
             << it.getProp("name") << endl;
    ...
    if (p.nbyte > 0) {
      mbcrit << myname
             << "(initRSProperty): config error: nbyte!=0 and mbmask!=0 for "
             << it.getProp("name") << endl;
...
// ------------------------------------------------------------------------------------------
bool MBConfig::initRegInfo(std::shared_ptr<RegInfo> &r, UniXML::iterator &it,
                           std::shared_ptr<MBConfig::RTUDevice> &dev) {
  r->dev = dev;
  ...
  if (dev->dtype == MBConfig::dtRTU) {
    ...
    UniversalIO::IOType t =
        uniset::getIOType(IOBase::initProp(it, "iotype", prop_prefix, false));
  ...
  if (mbregFromID) {
    ...
    if (sr.empty()) {
      mbcrit << myname << "(initItem): Unknown 'mbreg' for "
             << it.getProp("name") << endl;
      return false;
    }
  ...
  if (!f.empty()) {
    ...
    if (r->mbfunc == ModbusRTU::fnUnknown) {
      mbcrit << myname << "(initRegInfo): Unknown mbfunc ='" << f << "' for "
             << it.getProp("name") << endl;
      return false;
    }
  }
...
// ------------------------------------------------------------------------------------------
bool MBConfig::initRTUDevice(std::shared_ptr<RTUDevice> &d,
                             UniXML::iterator &it) {
  ...
  if (addr.empty()) {
    mbcrit << myname << "(initRTUDevice): Unknown mbaddr for "
           << it.getProp("name") << endl;
    return false;
  }
...
bool MBConfig::initItem(UniXML::iterator &it) {
  ...
  if (p.t_ai !=
      DefaultObjectId) // пороговые датчики в список обмена вносить не надо.
    return true;

  string addr =
      IOBase::initProp(it, "mbaddr", prop_prefix, false, defaultMBaddr);

  if (addr.empty()) {
    mbcrit << myname << "(initItem): Unknown mbaddr='" << addr << "' for "
           << it.getProp("name") << endl;
    return false;
  }
  ...
  if (!dev) {
    mbcrit << myname << "(initItem): " << it.getProp("name")
           << " CAN`T ADD for polling!" << endl;
    return false;
  }
  ...
  if (dev->dtype == dtRTU188) {
    ...
    if (!initRTU188item(it, r_tmp)) {
      mbcrit << myname << "(initItem): init RTU188 failed for "
             << it.getProp("name") << endl;
      r_tmp.reset();
      return false;
    }
    ...
    if (mbregFromID)
      ...
      if (reg.empty()) {
        mbcrit << myname << "(initItem): unknown mbreg(" << prop_prefix
               << ") for " << it.getProp("name") << endl;
        return false;
      }
    ...
    if (p.nbit != -1) {
      if (fn == ModbusRTU::fnReadCoilStatus ||
          fn == ModbusRTU::fnReadInputStatus) {
        mbcrit << myname
               << "(initItem): MISMATCHED CONFIGURATION!  nbit=" << (int)p.nbit
               << " func=" << fn << " for " << it.getProp("name") << endl;
        return false;
      }
    }
  }
   ...
   * 65535 - Отдельное значение для записи регистров по изменению значения
   * датчика. По-этому без оптимизации "склейки" регистров в один запрос.
   * TODO:
   * - На подумать, оптимизировать запросы если регистры для записи по изменению
   * идут подряд, в каких случаях это будет полезно.
   * - На подумать, сделать общий флаг для группы(всех датчиков), чтобы не
   * проверять все регистры по отдельности. Т.е. когда есть оптимизация,
   * отправляем все по изменению хотя бы одного датчика.
   */
  size_t pollfactor =
      IOBase::initIntProp(it, "pollfactor", prop_prefix, false, 0);

  if ((!ModbusRTU::isWriteFunction((ModbusRTU::SlaveFunctionCode)fn) ||
       dev->dtype != MBConfig::dtRTU) &&
      pollfactor == changeOnlyWrite) {
    ostringstream err;
    err << myname << "(initItem): " << it.getProp("name") << " for pollfactor("
        << changeOnlyWrite
  ...
  if (checkDuplicationRegID(rID, dev, rmap)) {
    ostringstream err;
    err << myname << "(checkDuplicationRegID): Found dublicate for RegID "
        << rID << " enable crit logs for see it ";
    throw uniset::SystemError(err.str());
  }

  // Проверка на пересечение с регистрами из нескольких слов.
  // Если регистр имеет тип размером в несколько слов, то проверяем что нет
  // дубликата регистра в одно слово. Для записи регистров из нескольких слов
  // задается соответствующая функция отличная от функции для обычных регистров
  // из одного слова. Поэтому проверка на дубликат выше не обнаружит пересечение
  // таких регистров. Для этого проверка ниже, которая также проверит на
  // пересечение регистры из нескольких слов между собой. Проверка только для
  // регистров на запись!
  ...
  if (fn == ModbusRTU::fnWriteOutputSingleRegister) {
    ...
    for (int i = 0; i < p.rnum; i++) {
      rID2 = ModbusRTU::genRegID(mbreg + i,
                                 ModbusRTU::fnWriteOutputSingleRegister);
      bad_status = checkDuplicationRegID(rID2, dev, nullptr);
  ...
  if (bad_status) {
    ostringstream err;
    err << myname << "(checkDuplicationRegID): Found dublicate for RegID "
        << rID2 << " enable crit logs to see it ";
    throw uniset::SystemError(err.str());
  }
  ...
  if (dev->dtype == dtMTR) {
    ...
    if (p.rnum <= 0) {
      mbcrit << myname << "(initItem): unknown word size for "
             << it.getProp("name") << endl;
      return false;
    }
  }
  ...
  // 4). Байтовый тип регистра на запись не обрабатывает несколько датчиков в
  // одном регистре т.е. для байтового типа может быть только один датчик на
  // регистр и соответственно только один "nbyte". Попытка привязать два датчика
  // с разными "nbyte" к одному регистру приведет к ошибке конфигурирования.
  // ИТОГ:
  // Если для функций записи список датчиков для регистра > 0
  // значит в списке уже есть либо целый датчик, либо битовый, либо датчик с
  // маской. Если идёт попытка внести в список целый датчик, то ОШИБКА! И
  // наоборот: если идёт попытка внести битовый датчик, а в списке уже сидит
  // датчик занимающий целый регистр, то тоже ОШИБКА!
  ...
  if (ModbusRTU::isWriteFunction(ri->mbfunc)) {
    if (p.nbit < 0 && p.mask == 0 && ri->slst.size() > 0) {
      ...
      ostringstream err;
      err << myname
          << "(initItem): FAILED! Sharing SAVE (not bit or mask saving) to "
    ...
    // - либо общий тег "init_mbval_changed" на всех в настроечной секции
    // процесса. Тоже и для параметра --prefix-init-mbval-changed в командной
    // строке при запуске. Если mbval_changed=true, регистр запишется сразу
    // после запуска, иначе только после изменения значения датчика.
    ri->mbval_changed = IOBase::initIntProp(
        it, "init_mbval_changed", prop_prefix, false, init_mbval_changed);
    ri->mb_initOK = true;
  } else {
    ...
    for (auto i = 1; i < p1->rnum; i++) {
      ...
      if (ModbusRTU::isWriteFunction(ri->mbfunc)) {
        ...
        if (ri->mbfunc != ModbusRTU::fnWriteOutputRegisters) {
          ostringstream err;
          err << myname << "(initItem): Bad write function ='"
              << ModbusRTU::fnWriteOutputSingleRegister << "' for vtype='"
              << p1->vType << "'"
              << " mbreg=" << ModbusRTU::dat2str(ri->mbreg) << " for "
              << it.getProp("name");
...
bool MBConfig::initMTRitem(UniXML::iterator &it, std::shared_ptr<RegInfo> &p) {
  ...
  if (p->mtrType == MTR::mtUnknown) {
    mbcrit << myname << "(readMTRItem): Unknown mtrtype '"
           << it.getProp(prop_prefix + "mtrtype") << "' for "
           << it.getProp("name") << endl;
...
// ------------------------------------------------------------------------------------------
bool MBConfig::initRTU188item(UniXML::iterator &it,
                              std::shared_ptr<RegInfo> &p) {
  ...
  if (jack.empty()) {
    mbcrit << myname << "(readRTU188Item): Unknown " << prop_prefix
           << "jack='' "
           << " for " << it.getProp("name") << endl;
    return false;
  }
  ...
  if (p->rtuJack == RTUStorage::nUnknown) {
    mbcrit << myname << "(readRTU188Item): Unknown " << prop_prefix
           << "jack=" << jack << " for " << it.getProp("name") << endl;
    return false;
  }
...
std::ostream &operator<<(std::ostream &os, const MBConfig::RSProperty &p) {
  os << " (" << ModbusRTU::dat2str(p.reg->mbreg) << ")"
     << " sid=" << p.si.id << " stype=" << p.stype << " nbit=" << (int)p.nbit
     << " nbyte=" << p.nbyte << " rnum=" << p.rnum << " safeval=" << p.safeval
     << " invert=" << p.invert;

  if (p.stype == UniversalIO::AI || p.stype == UniversalIO::AO) {
    os << p.cal << " cdiagram=" << (p.cdiagram ? "yes" : "no");
  }
...
// -----------------------------------------------------------------------------
bool MBConfig::initDeviceInfo(RTUDeviceMap &m, ModbusRTU::ModbusAddr a,
                              UniXML::iterator &it) {
  auto d = m.find(a);

  if (d == m.end()) {
    mbwarn << myname << "(initDeviceInfo): not found device for addr="
           << ModbusRTU::addr2str(a) << endl;
    return false;
  }
  ...
  if (!s.empty()) {
    ...
    if (dev->resp_id == DefaultObjectId) {
      mbinfo << myname
             << "(initDeviceInfo): not found ID for respondSensor=" << s
             << endl;
      return false;
    }
  }
  ...
  if (!mod.empty()) {
    ...
    if (dev->mode_id == DefaultObjectId) {
      mbcrit << myname
             << "(initDeviceInfo): not found ID for modeSensor=" << mod << endl;
      return false;
    }
    ...
    if (m_iotype != UniversalIO::AI) {
      mbcrit << myname << "(initDeviceInfo): modeSensor='" << mod
             << "' must be 'AI'" << endl;
      return false;
    }
  }
  ...
  if (!safemode.empty()) {
    ...
    if (dev->safemode_id == DefaultObjectId) {
      mbcrit << myname
             << "(initDeviceInfo): not found ID for safemodeSensor=" << safemode
             << endl;
      return false;
    }
...
// -----------------------------------------------------------------------------
bool MBConfig::checkDuplicationRegID(
    const ModbusRTU::RegID id, const std::shared_ptr<RTUDevice> &dev,
  ...
  for (const auto &d : dev->pollmap) {
    ...
    if (d.second->find(id) != d.second->end()) {
      mbcrit << myname << "(checkDuplicationRegID): Found dublicate for RegID "
             << id << " mbreg " << (*d.second)[id]->mbreg << " mbfunc "
             << (*d.second)[id]->mbfunc << " in pollmap " << d.first << endl;
      ret = true;
    }
  }
...
bool MBConfig::RTUDevice::checkRespond(std::shared_ptr<DebugStream> &mblog) {
  ...
  mblog4 << "(checkRespond): addr=" << ModbusRTU::addr2str(mbaddr)
         << " respond_id=" << resp_id << " state=" << resp_state
         << " check=" << (prev_numreply != numreply)
         << " delay_check=" << resp_Delay.get()
         << " [ timeout=" << resp_Delay.getOffDelay()
         << " numreply=" << numreply << " prev_numreply=" << prev_numreply
         << " resp_ptInit=" << resp_ptInit.checkTime() << " ]" << endl;
...
std::string MBConfig::RTUDevice::getShortInfo() const {
  ...
  s << "mbaddr=" << ModbusRTU::addr2str(mbaddr) << ":"
    << " resp_state=" << resp_state << " (resp_id=" << resp_id
    << " resp_force=" << resp_force << " resp_invert=" << resp_invert
    << " numreply=" << numreply << " timeout=" << resp_Delay.getOffDelay()
    << " type=" << dtype << " ask_every_reg=" << ask_every_reg
    << " stopBits=" << stopBits << " charSize=" << csize << " parity=" << parity
    << " regs=" << regs << ")" << endl;
  return s.str();
}
// -----------------------------------------------------------------------------
std::string MBConfig::getShortInfo() const {
  ...
  s << " recv_timeout=" << recv_timeout
    ...
    << " aftersend_pause=" << aftersend_pause << " polltime=" << polltime
    << " sleepPause_msec=" << sleepPause_msec
    << " maxQueryCount=" << maxQueryCount
    << " noQueryOptimization=" << noQueryOptimization << " prefix=" << prefix
    << " prop_prefix=" << prop_prefix << " s_field=" << s_field
    << " s_fvalue=" << s_fvalue << " defaultMBtype=" << defaultMBtype
    << " defaultMBaddr=" << defaultMBaddr << " mbregFromID=" << mbregFromID
...
std::string to_string(const uniset::MBConfig::PList &lst) {
  ...
  for (const auto &p : lst)
    s << "(" << p.si.id << ")"
      << conf->oind->getBaseName(conf->oind->getMapName(p.si.id)) << " ";

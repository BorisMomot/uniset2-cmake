...
MBTCPMaster::MBTCPMaster(uniset::ObjectId objId, uniset::ObjectId shmId,
  ...
  if (objId == DefaultObjectId)
    throw uniset::SystemError("(MBTCPMaster): objId=-1?!! Use --" + prefix +
                              "-name");
  ...
  if (iaddr.empty())
    throw uniset::SystemError(
        myname + "(MBMaster): Unknown inet addr...(Use: " + pname + ")");
  ...
  if (port <= 0)
    throw uniset::SystemError(
        myname + "(MBMaster): Unknown inet port...(Use: " + tmp + ")");
  ...
  force_disconnect = conf->getArgInt("--" + prefix + "-persistent-connection",
                         ...
                         ? false
                         : true;
  mbinfo << myname << "(init): persistent-connection=" << (!force_disconnect)
         << endl;
  ...
  pollThread = unisetstd::make_unique<ThreadCreator<MBTCPMaster>>(
      this, &MBTCPMaster::poll_thread);
...
void MBTCPMaster::help_print(int argc, const char *const *argv) {
  ...
  cout << "--prefix-gateway-iaddr hostname,IP     - IP опрашиваемого узла"
       << endl;
  cout << "--prefix-gateway-port num              - port на опрашиваемом узле"
       << endl;
...
// -----------------------------------------------------------------------------
std::shared_ptr<MBTCPMaster> MBTCPMaster::init_mbmaster(
...
uniset::SimpleInfo *MBTCPMaster::getInfo(const char *userparam) {
  ...
  inf << i->info << endl;
  inf << "poll: " << iaddr << ":" << port
      << " pesrsistent-connection=" << (force_disconnect ? "NO" : "YES")
      << endl;

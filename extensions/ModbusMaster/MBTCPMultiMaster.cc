...
// -----------------------------------------------------------------------------
MBTCPMultiMaster::MBTCPMultiMaster(uniset::ObjectId objId,
                                   uniset::ObjectId shmId,
                                   const std::shared_ptr<SharedMemory> &ic,
                                   const std::string &prefix)
    : MBExchange(objId, shmId, ic, prefix), force_disconnect(true) {
  if (objId == DefaultObjectId)
    throw uniset::SystemError("(MBTCPMultiMaster): objId=-1?!! Use --" +
                              prefix + "-name");
  ...
  checktime = conf->getArgPInt("--" + prefix + "-checktime",
                               it.getProp("checkTime"), 5000);
  force_disconnect = conf->getArgInt("--" + prefix + "-persistent-connection",
                                     it.getProp("persistent_connection"))
                         ? false
                         : true;

  defaultIgnoreTimeout =
      conf->getArgPInt("--" + prefix + "-ignore-timeout",
                       it.getProp("ignoreTimeout"), ptReopen.getInterval());

  // Т.к. при "многоканальном" доступе к slave, смена канала должна происходит
  // сразу после неудачной попытки запросов по одному из каналов, то
  // ПЕРЕОПРЕДЕЛЯЕМ reopen, на channel-timeout..
  channelTimeout =
      conf->getArgPInt("--" + prefix + "-default-channel-timeout",
                       it.getProp("channelTimeout"), mbconf->default_timeout);
  ...
  pollThread = unisetstd::make_unique<ThreadCreator<MBTCPMultiMaster>>(
      this, &MBTCPMultiMaster::poll_thread);
  pollThread->setFinalAction(this, &MBTCPMultiMaster::final_thread);
  checkThread = unisetstd::make_unique<ThreadCreator<MBTCPMultiMaster>>(
      this, &MBTCPMultiMaster::check_thread);
...
// -----------------------------------------------------------------------------
void MBTCPMultiMaster::initGateList(
    uniset::UniXML::iterator it,
    const std::shared_ptr<uniset::MBConfig> &mconf) {
  auto conf = uniset_conf();
  ...
  for (; it1.getCurrent(); it1++) {
    if (it1.getIntProp("ignore")) {
      mbinfo << myname << "(init): IGNORE " << it1.getProp("ip") << ":"
             << it1.getProp("port") << endl;
      continue;
    }
    ...
    if (sinf->port <= 0) {
      ostringstream err;
      err << myname << "(init): ERROR: port=''" << sinf->port << " for ip='"
          << sinf->ip << "' in <GateList>";
      mbcrit << err.str() << endl;
      throw uniset::SystemError(err.str());
    }
    ...
    if (!it1.getProp("respondSensor").empty()) {
      ...
      if (sinf->respond_id == DefaultObjectId) {
        ostringstream err;
        err << myname << "(init): ERROR: Unknown SensorID for '"
            << it1.getProp("respondSensor") << "' in <GateList>";
        mbcrit << err.str() << endl;
        throw uniset::SystemError(err.str());
      }
    }
    ...
    sinf->ptIgnoreTimeout.setTiming(
        it1.getPIntProp("ignoreTimeout", defaultIgnoreTimeout));
    sinf->recv_timeout = it1.getPIntProp("recv_timeout", mconf->recv_timeout);
    sinf->aftersend_pause =
        it1.getPIntProp("aftersendPause", mconf->aftersend_pause);
    sinf->sleepPause_usec =
        it1.getPIntProp("sleepPause_msec", mconf->sleepPause_msec);
    sinf->respond_invert = it1.getPIntProp("invert", 0);
    sinf->respond_force = it1.getPIntProp("force", 0);

    int fn = conf->getArgPInt("--" + mconf->prefix + "-check-func",
                              it.getProp("checkFunc"), ModbusRTU::fnUnknown);

    if (fn != ModbusRTU::fnUnknown && fn != ModbusRTU::fnReadCoilStatus &&
        fn != ModbusRTU::fnReadInputStatus &&
        fn != ModbusRTU::fnReadOutputRegisters &&
        fn != ModbusRTU::fnReadInputRegisters) {
      ostringstream err;
      err << myname << "(init):  BAD check function ='" << fn
          << "'. Must be [1,2,3,4]";
      mbcrit << err.str() << endl;
    ...
    sinf->checkAddr = conf->getArgPInt("--" + mconf->prefix + "-check-addr",
                                       it.getProp("checkAddr"), 0);
    sinf->checkReg = conf->getArgPInt("--" + mconf->prefix + "-check-reg",
                                      it.getProp("checkReg"), 0);
    ...
    sinf->force_disconnect =
        it.getPIntProp("persistent_connection", !force_disconnect) ? false
                                                                   : true;
...
std::shared_ptr<ModbusClient> MBTCPMultiMaster::initMB(bool reopen) {
...

  {
    ...
    // если reopen=true - значит почему-то по текущему каналу связи нет (хотя
    // соединение есть) тогда выставляем ему признак игнорирования
    if (mbi != mblist.rend() && reopen) {
      auto m = (*mbi);
      m->setUse(false);
      m->ignore = true;
      m->ptIgnoreTimeout.reset();
      mbwarn << myname << "(initMB): set ignore=true for " << m->ip << ":"
             << m->port << endl;
    }

    // Если по текущему каналу связь есть (и мы его не игнорируем), то
    // возвращаем его
    if (mbi != mblist.rend() && !(*mbi)->ignore && (*mbi)->respond) {
      ...
      if (m->respond && (m->mbtcp->isConnection() || m->init(mblog))) {
        mblog4 << myname << "(initMB): SELECT CHANNEL " << m->ip << ":"
               << m->port << endl;
        mb = m->mbtcp;
        m->setUse(true);
        ptReopen.setTiming(m->channel_timeout);
  ...
  for (auto it = mblist.rbegin(); it != mblist.rend(); ++it) {
    ...
    if (m->respond && !m->ignore && m->init(mblog)) {
      ...
      m->setUse(true);
      ptReopen.setTiming(m->channel_timeout);
      mblog4 << myname << "(initMB): SELECT CHANNEL " << m->ip << ":" << m->port
             << endl;
      return m->mbtcp;
    }
  }
...
bool MBTCPMultiMaster::MBSlaveInfo::check() {
  ...
  if (!lock.owns_lock())
    return use; // возвращаем 'use' т.к. если use=1 то считается что связь
                // есть..
...
bool MBTCPMultiMaster::MBSlaveInfo::init(std::shared_ptr<DebugStream> &mblog) {
  ...
  mbinfo << myname << "(init): connect "
         << (mbtcp->isConnection() ? "OK" : "FAIL") << endl;
  ...
  while (isProcActive()) {
    for (auto &&it : mblist) {
      try {
        ...
        mblog4 << myname << "(check): " << it->myname << " " << setw(4)
               << (r ? "OK" : "FAIL")
               << " [ respondDelay=" << it->respondDelay.check(r)
               << " timeout=" << it->channel_timeout << " use=" << it->use
               << " ignore=" << it->ignore << " respond_id=" << it->respond_id
               << " respond_force=" << it->respond_force
               << " respond=" << it->respond
               << " respond_invert=" << it->respond_invert
               << " activated=" << isProcActive() << " ]" << endl;
        ...
        try {
          if (it->respond_id != DefaultObjectId &&
              (it->respond_force || !it->respond_init || r != it->respond)) {
            bool set = it->respond_invert ? !r : r;
            shm->localSetValue(it->respond_it, it->respond_id, (set ? 1 : 0),
                               getId());
          ...
          mbcrit << myname << "(check): (respond) " << it->myname << " : " << ex
                 << std::endl;
        } catch (const std::exception &ex) {
          mbcrit << myname << "(check): (respond) " << it->myname << " : "
                 << ex.what() << std::endl;
        }
...
void MBTCPMultiMaster::initCheckConnectionParameters() {
  ...
  bool initFromRegMap =
      (findArgParam("--" + mbconf->prefix + "-check-init-from-regmap",
                    conf->getArgc(), conf->getArgv()) != -1);

  if (!initFromRegMap)
    return;

  mbinfo << myname << "(init): init check connection parameters from regmap.."
         << endl;
  ...
  for (const auto &d : mbconf->devices) {
    ...
    for (auto p = d.second->pollmap.begin(); p != d.second->pollmap.end();
         ++p) {
  ...
  if (checkFunc == ModbusRTU::fnUnknown) {
    ...
    err << myname << "(init): init check connection parameters: ERROR: "
        << " NOT FOUND read-registers for check connection!" << endl;
...
void MBTCPMultiMaster::help_print(int argc, const char *const *argv) {
  ...
  cout << "--prefix-persistent-connection 0,1 - Не закрывать соединение на "
          "каждом цикле опроса"
       << endl;
  cout << "--prefix-checktime msec            - период проверки связи по "
          "каналам (<GateList>)"
       << endl;
  cout << "--prefix-ignore-timeout msec       - Timeout на повторную попытку "
          "использования канала после 'reopen-timeout'. По умолчанию: "
          "reopen-timeout * 3"
       << endl;

  cout << "--prefix-check-func [1,2,3,4]      - Номер функции для проверки "
          "соединения"
       << endl;
  cout << "--prefix-check-addr [1..255 ]      - Адрес устройства для проверки "
          "соединения"
       << endl;
  cout << "--prefix-check-reg [1..65535]      - Регистр для проверки соединения"
       << endl;

  cout << endl;
  cout << " ВНИМАНИЕ! '--prefix-reopen-timeout' для MBTCPMultiMaster НЕ "
          "ДЕЙСТВУЕТ! "
       << endl;
  cout << " Смена канала происходит по --prefix-timeout." << endl;
  cout << " При этом следует учитывать блокировку отключаемого канала на "
          "время: --prefix-ignore-timeout"
       << endl;
}
// -----------------------------------------------------------------------------
std::shared_ptr<MBTCPMultiMaster> MBTCPMultiMaster::init_mbmaster(
    int argc, const char *const *argv, uniset::ObjectId icID,
...
const std::string MBTCPMultiMaster::MBSlaveInfo::getShortInfo() const {
  ostringstream s;
  s << myname << " respond=" << respond << " (respond_id=" << respond_id
    << " respond_invert=" << respond_invert << " recv_timeout=" << recv_timeout
    << " resp_force=" << respond_force << " use=" << use
...
uniset::SimpleInfo *MBTCPMultiMaster::getInfo(const char *userparam) {
  ...
  inf << i->info << endl;
  inf << "Gates: " << (checktime <= 0 ? "/ check connections DISABLED /" : "")
      << endl;
...
// ----------------------------------------------------------------------------
bool MBTCPMultiMaster::reconfigure(
    const std::shared_ptr<uniset::UniXML> &xml,
    const std::shared_ptr<uniset::MBConfig> &newConf) {
  ...
  if (!it.getProp("persistent_connection").empty())
    force_disconnect =
        (it.getIntProp("persistent_connection") > 0) ? false : true;

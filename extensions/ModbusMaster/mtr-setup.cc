...
static void print_help() {
  ...
  printf("[-s|--speed] speed        - 9600,14400,19200,38400,57600,115200. "
         "Default: 38400.\n");
  ...
  printf(
      "          fn - function of test [0x01,0x02,0x03,0x04]. Default: 0x04\n");
  printf("\nTCP prameters:\n");
  printf("[-i|--iaddr] ip                 - Modbus server ip. Default: "
         "127.0.0.1\n");
  printf(
  ...
  try {
    while (1) {
      opt = getopt_long(argc, argv,
                        "hvw:r:x:d:s:t:l:n:yb:e:x:z:i:p:o:a:", longopts,
                        &optindex);
...
#undef atoi
    ...
    if (verb) {
      cout << "(init): dev=" << dev << " speed=" << speed << " timeout=" << tout
           << " msec " << endl;
    }
    ...
    case cmdSave: {
      if (slaveaddr == 0x00) {
        if (verb)
          cout << "(mtr-setup): save: autodetect slave addr... (speed=" << speed
               << ")" << endl;

        mb->setTimeout(50);
        slaveaddr = ModbusHelpers::autodetectSlave(
            mb, beg, end, MTR::regModelNumber, ModbusRTU::fnReadInputRegisters);
        mb->setTimeout(tout);
      }

      if (speed.empty()) {
        if (verb)
          cout << "(mtr-setup): save: autodetect speed... (addr="
               << ModbusRTU::addr2str(slaveaddr) << ")" << endl;
        ...
        if (mbrtu) {
          mb->setTimeout(50);
          ComPort::Speed s = ModbusHelpers::autodetectSpeed(
              mbrtu, slaveaddr, MTR::regModelNumber,
              ModbusRTU::fnReadInputRegisters);
          mbrtu->setSpeed(s);
      ...
      if (verb)
        cout << "(mtr-setup): save: "
             ...
             << " confile=" << mtrconfile << " speed=" << speed << endl;
    ...
    case cmdDetectSlave: {
      if (verb) {
        cout << "(mtr-setup): autodetect slave: "
             ...
             << " fn=" << ModbusRTU::b2str(fn) << endl;
      }

      try {
        ModbusRTU::ModbusAddr a =
            ModbusHelpers::autodetectSlave(mb, beg, end, reg, fn);
        cout << "(mtr-setup): autodetect modbus slave: "
    ...
    case cmdDetectSpeed: {
      ...
      if (verb) {
        cout << "(mtr-setup): autodetect speed: slaveaddr="
             << ModbusRTU::addr2str(slaveaddr)
             << " reg=" << ModbusRTU::dat2str(reg)
             << " fn=" << ModbusRTU::b2str(fn) << endl;
      }
      ...
      try {
        ComPort::Speed s =
            ModbusHelpers::autodetectSpeed(mbrtu, slaveaddr, reg, fn);
        cout << "(mtr-setup): autodetect: slaveaddr="
             << ModbusRTU::addr2str(slaveaddr)
    ...
    case cmdGetModel: {
      if (verb) {
        cout << "(mtr-setup): model: "
             << " slaveaddr=" << ModbusRTU::addr2str(slaveaddr) << endl;
      }

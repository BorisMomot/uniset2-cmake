...
MBExchange::MBExchange(uniset::ObjectId objId, uniset::ObjectId shmId,
                       const std::shared_ptr<SharedMemory> &_ic,
                       const std::string &prefix)
    : UniSetObject(objId), allInitOK(false), force(false), force_out(false),
      sidExchangeMode(DefaultObjectId), exchangeMode(MBConfig::emNone),
      activated(false), notUseExchangeTimer(false), poll_count(0), mb(nullptr),
      ic(_ic) {
  if (objId == DefaultObjectId)
    throw uniset::SystemError("(MBExchange): objId=-1?!! Use --" + prefix +
                              "-name");
  ...
  if (cnode == NULL)
    throw uniset::SystemError("(MBExchange): Not found node <" + conf_name +
                              " ...> for " + myname);
  ...
  if (findArgParam("--" + prefix + "-run-logserver", conf->getArgc(),
                   conf->getArgv()) != -1) {
    logserv_host = conf->getArg2Param("--" + prefix + "-logserver-host",
                                      it.getProp("logserverHost"), "localhost");
    logserv_port = conf->getArgPInt("--" + prefix + "-logserver-port",
                                    it.getProp("logserverPort"), getId());
  }
  ...
  // определяем фильтр
  mbconf->s_field = conf->getArg2Param("--" + prefix + "-filter-field",
                                       it.getProp("filterField"));
  mbconf->s_fvalue = conf->getArg2Param("--" + prefix + "-filter-value",
                                        it.getProp("filterValue"));
  ...
  stat_time = conf->getArgPInt("--" + prefix + "-statistic-sec",
                               it.getProp("statistic_sec"), 0);
  vmonit(stat_time);

  if (stat_time > 0)
    ptStatistic.setTiming(stat_time * 1000);

  mbconf->recv_timeout = conf->getArgPInt("--" + prefix + "-recv-timeout",
                                          it.getProp("recv_timeout"), 500);
  mbconf->default_timeout =
      conf->getArgPInt("--" + prefix + "-timeout", it.getProp("timeout"), 5000);

  int tout = conf->getArgPInt("--" + prefix + "-reopen-timeout",
                              it.getProp("reopen_timeout"),
                              mbconf->default_timeout * 2);
  ptReopen.setTiming(tout);

  int reinit_tout =
      conf->getArgPInt("--" + prefix + "-reinit-timeout",
                       it.getProp("reinit_timeout"), mbconf->default_timeout);
  ptInitChannel.setTiming(reinit_tout);

  mbconf->aftersend_pause = conf->getArgPInt("--" + prefix + "-aftersend-pause",
                                             it.getProp("aftersendPause"), 0);
  mbconf->noQueryOptimization =
      conf->getArgInt("--" + prefix + "-no-query-optimization",
                      it.getProp("noQueryOptimization"));
  mbconf->mbregFromID =
      conf->getArgInt("--" + prefix + "-reg-from-id", it.getProp("regFromId"));
  mbinfo << myname << "(init): mbregFromID=" << mbconf->mbregFromID << endl;

  mbconf->init_mbval_changed =
      conf->getArgPInt("--" + prefix + "-init-mbval-changed",
                       it.getProp("init_mbval_changed"), 1)
          ? true
          : false;

  mbconf->polltime = conf->getArgPInt("--" + prefix + "-polltime",
                                      it.getProp("polltime"), 100);
  initPause = conf->getArgPInt("--" + prefix + "-init-pause",
                               it.getProp("initPause"), 3000);

  mbconf->sleepPause_msec = conf->getArgPInt("--" + prefix + "-sleepPause-msec",
                                             it.getProp("sleepPause"), 10);

  force = conf->getArgInt("--" + prefix + "-force", it.getProp("force"));
  force_out =
      conf->getArgInt("--" + prefix + "-force-out", it.getProp("force_out"));
  vmonit(force);
  vmonit(force_out);

  mbconf->defaultMBtype = conf->getArg2Param(
      "--" + prefix + "-default-mbtype", it.getProp("default_mbtype"), "rtu");
  mbconf->defaultMBaddr = conf->getArg2Param("--" + prefix + "-default-mbaddr",
                                             it.getProp("default_mbaddr"), "");
  mbconf->defaultMBinitOK = conf->getArgPInt(
      "--" + prefix + "-default-mbinit-ok", it.getProp("default_mbinitOK"), 0);
  mbconf->maxQueryCount =
      conf->getArgPInt("--" + prefix + "-query-max-count",
                       it.getProp("queryMaxCount"), ModbusRTU::MAXDATALEN);

  // ********** HEARTBEAT *************
  string heart = conf->getArgParam("--" + prefix + "-heartbeat-id",
                                   it.getProp("heartbeat_id"));
  ...
  if (!heart.empty()) {
    ...
    int heartbeatTime =
        conf->getArgPInt("--" + prefix + "-heartbeat-time",
                         it.getProp("heartbeatTime"), conf->getHeartBeatTime());
    ...
    maxHeartBeat = conf->getArgPInt("--" + prefix + "-heartbeat-max",
                                    it.getProp("heartbeat_max"), 10);
  }

  string sm_ready_sid = conf->getArgParam(
      "--" + prefix + "-sm-test-sid", it.getProp2("smTestSID", "TestMode_S"));
  sidTestSMReady = conf->getSensorID(sm_ready_sid);
  mbinfo << myname << "(init): sidTestSMReady=" << sidTestSMReady << endl;
  vmonit(sidTestSMReady);

  string emode = conf->getArgParam("--" + prefix + "-exchange-mode-id",
                                   it.getProp("exchangeModeID"));
  ...
  activateTimeout =
      conf->getArgPInt("--" + prefix + "-activate-timeout", 20000);
...
std::string MBExchange::initPropPrefix(const std::string &def_prop_prefix) {
  ...
  if (!v.empty() && v[0] != '-')
    pp = v;
  // если параметр всё-таки указан, считаем, что это попытка задать "пустой"
  // префикс
...
void MBExchange::help_print(int argc, const char *const *argv) {
  cout << "--prefix-name name              - ObjectId (имя) процесса. По "
          "умолчанию: MBExchange1"
       << endl;
  cout << "--prefix-confnode name          - Настроечная секция в конф. файле "
          "<name>. "
       << endl;
  cout << "--prefix-polltime msec          - Пауза между опросами. По "
          "умолчанию 100 мсек."
       << endl;
  cout << "--prefix-recv-timeout msec      - Таймаут на приём одного сообщения"
       << endl;
  cout << "--prefix-timeout msec           - Таймаут для определения "
          "отсутствия соединения"
       << endl;
  cout << "--prefix-aftersend-pause msec   - Пауза после посылки запроса "
          "(каждого). По умолчанию: 0."
       << endl;
  cout << "--prefix-reopen-timeout msec    - Таймаут для 'переоткрытия "
          "соединения' при отсутствия соединения msec миллисекунд. По "
          "умолчанию 10 сек."
       << endl;
  cout << "--prefix-heartbeat-id  name     - Данный процесс связан с указанным "
          "аналоговым heartbeat-датчиком."
       << endl;
  cout << "--prefix-heartbeat-max val      - Максимальное значение "
          "heartbeat-счётчика для данного процесса. По умолчанию 10."
       << endl;
  cout << "--prefix-force 0,1              - Сохранять значения в SM на каждом "
          "шаге, независимо от, того менялось ли значение"
       << endl;
  cout << "--prefix-force-out 0,1          - Считывать значения 'выходов' из "
          "SM на каждом шаге (а не по изменению)"
       << endl;
  cout << "--prefix-initPause msec         - Задержка перед инициализацией "
          "(время на активизация процесса)"
       << endl;
  cout << "--prefix-no-query-optimization 0,1 - Не оптимизировать запросы (не "
          "объединять соседние регистры в один запрос)"
       << endl;
  cout << "--prefix-reg-from-id 0,1        - Использовать в качестве регистра "
          "sensor ID"
       << endl;
  cout << "--prefix-filter-field name      - Считывать список опрашиваемых "
          "датчиков, только у которых есть поле field"
       << endl;
  cout << "--prefix-filter-value val       - Считывать список опрашиваемых "
          "датчиков, только у которых field=value"
       << endl;
  cout << "--prefix-statistic-sec sec      - Выводить статистику опроса каждые "
          "sec секунд"
       << endl;
  cout << "--prefix-sm-ready-timeout msec     - Время ожидания готовности SM к "
          "работе, мсек. (-1 - ждать 'вечно')"
       << endl;
  cout << "--prefix-sm-ready-test-sid name - Датчик для проверки готовности SM "
          "к работе. По умолчанию идёт попытка автоопределения."
       << endl;
  cout << "--prefix-exchange-mode-id       - Идентификатор (AI) датчика, "
          "позволяющего управлять работой процесса"
       << endl;
  cout << "--prefix-set-prop-prefix val    - Использовать для свойств "
          "указанный или пустой префикс."
       << endl;
  cout << "--prefix-default-mbtype [rtu|rtu188|mtr]  - У датчиков которых не "
          "задан 'mbtype' использовать данный. По умолчанию: 'rtu'"
       << endl;
  cout << "--prefix-default-mbadd addr     - У датчиков которых не задан "
          "'mbaddr' использовать данный. По умолчанию: ''"
       << endl;
  cout << "--prefix-default-mbinit-ok 0,1  - Флаг инициализации. 1 - не ждать "
          "первого обмена с устройством, а сохранить при старте в SM значение "
          "'default'"
       << endl;
  cout << "--prefix-query-max-count max    - Максимальное количество "
          "запрашиваемых за один раз регистров (При условии "
          "no-query-optimization=0). По умолчанию: "
       << ModbusRTU::MAXDATALEN << "." << endl;
  cout << "--prefix-init-mbval-changed 0,1 - Инициализация флага изменения "
          "значения датчика регистра. Значение по умолчанию при запуске "
          "процесса."
       << endl;
  cout << endl;
  cout << " Logs: " << endl;
  ...
  cout << "             no-debug " << endl;
  cout << " LogServer: " << endl;
  cout << "--prefix-run-logserver      - run logserver. Default: localhost:id"
       << endl;
...
bool MBExchange::waitSMReady() {
  ...
  int tout =
      uniset_conf()->getArgInt("--" + mbconf->prefix + "-sm-ready-timeout", "");
  ...
  if (!shm->waitSMreadyWithCancellation(ready_timeout, canceled, 50)) {
    if (!canceled) {
      ostringstream err;
      err << myname << "(waitSMReady): failed waiting SharedMemory "
          << ready_timeout << " msec. ==> TERMINATE!";
      mbcrit << err.str() << endl;
    }
...
// -----------------------------------------------------------------------------
bool MBExchange::isProcActive() const { return activated && !canceled; }
// -----------------------------------------------------------------------------
void MBExchange::setProcActive(bool st) { activated = st; }
// -----------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------------------
bool MBExchange::readItem(const std::shared_ptr<UniXML> &xml,
  ...
  if (uniset::check_filter(it, mbconf->s_field, mbconf->s_fvalue)) {
    try {
      if (!mbconf->initItem(it)) {
        ostringstream err;
        err << myname
            << "(readItem): Error during sensor configuration name -> "
            << it.getProp("name");
        throw SystemError(err.str());
      }
...
void MBExchange::initIterators() {
  ...
  for (auto it1 = mbconf->devices.begin(); it1 != mbconf->devices.end();
       ++it1) {
    auto d = it1->second;
    ...
    for (auto &&m : d->pollmap) {
      ...
      for (auto it = regmap->begin(); it != regmap->end(); ++it) {
        for (auto it2 = it->second->slst.begin(); it2 != it->second->slst.end();
             ++it2) {
...
bool MBExchange::isUpdateSM(bool wrFunc, long mdev) const noexcept {
  if (exchangeMode == MBConfig::emSkipExchange ||
      mdev == MBConfig::emSkipExchange) {
    if (wrFunc)
      return true; // данные для посылки, должны обновляться всегда (чтобы быть
                   // актуальными, когда режим переключиться обратно..)
  ...
  if (wrFunc &&
      (exchangeMode == MBConfig::emReadOnly || mdev == MBConfig::emReadOnly)) {
    mblog3 << "(checkUpdateSM):"
           << " skip... mode='emReadOnly' " << endl;
    return false;
  }

  if (!wrFunc && (exchangeMode == MBConfig::emWriteOnly ||
                  mdev == MBConfig::emWriteOnly)) {
    mblog3 << "(checkUpdateSM):"
           << " skip... mode='emWriteOnly' " << endl;
    return false;
  }

  if (!wrFunc && (exchangeMode == MBConfig::emSkipSaveToSM ||
                  mdev == MBConfig::emSkipSaveToSM)) {
    mblog3 << "(checkUpdateSM):"
...
bool MBExchange::isPollEnabled(bool wrFunc) const noexcept {
  ...
  if (exchangeMode == MBConfig::emSkipExchange) {
    mblog3 << myname << "(checkPoll): skip.. poll mode='emSkipExchange'"
           << endl;
    return false;
  }
...
// -----------------------------------------------------------------------------
bool MBExchange::isSafeMode(
    std::shared_ptr<MBConfig::RTUDevice> &dev) const noexcept {
  if (!dev)
    return false;
  ...
  // поэтому проверка на "0" (0 - связи нет), а значит должен включиться
  // safeMode
...
void MBExchange::firstInitRegisters() {
  ...
  for (auto it = mbconf->initRegList.begin(); it != mbconf->initRegList.end();
       ++it) {
    try {
      ...
      allInitOK = false;
      mblog3 << myname << "(preInitRead): FAILED ask addr="
             << ModbusRTU::addr2str(it->dev->mbaddr)
             << " reg=" << ModbusRTU::dat2str(it->mbreg) << " err: " << ex
             << endl;
...
bool MBExchange::preInitRead(MBConfig::InitList::iterator &p) {
  mbinfo << "init mbreg=" << ModbusRTU::dat2str(p->mbreg)
         << " initOK=" << p->initOK << endl;
  ...
  case ModbusRTU::fnReadOutputRegisters: {
    ModbusRTU::ReadOutputRetMessage ret =
        mb->read03(dev->mbaddr, p->mbreg, q_count);
    p->initOK = initSMValue(ret.data, ret.count, &(p->p));
  } break;

  case ModbusRTU::fnReadInputRegisters: {
    ModbusRTU::ReadInputRetMessage ret1 =
        mb->read04(dev->mbaddr, p->mbreg, q_count);
    p->initOK = initSMValue(ret1.data, ret1.count, &(p->p));
  } break;

  case ModbusRTU::fnReadInputStatus: {
    ModbusRTU::ReadInputStatusRetMessage ret =
        mb->read02(dev->mbaddr, p->mbreg, q_count);
  ...
  case ModbusRTU::fnReadCoilStatus: {
    ModbusRTU::ReadCoilRetMessage ret =
        mb->read01(dev->mbaddr, p->mbreg, q_count);
    ModbusRTU::DataGuard d(q_count);
    size_t m = 0;
...
// -----------------------------------------------------------------------------
bool MBExchange::initSMValue(ModbusRTU::ModbusData *data, int count,
                             MBConfig::RSProperty *p) {
  ...
  try {
    if (p->vType == VTypes::vtUnknown) {
      ...
      if (p->nbyte <= 0 || p->nbyte > VTypes::Byte::bsize) {
        mbcrit << myname << "(initSMValue): IGNORE item: sid="
               << ModbusRTU::dat2str(p->si.id) << " vtype=" << p->vType
               << " but nbyte=" << p->nbyte << endl;
        return false;
      }
    ...
    mblog3 << myname
           << "(initSMValue): CORBA::SystemException: " << ex.NP_minorString()
           << endl;
  } catch (const uniset::Exception &ex) {
...
// -----------------------------------------------------------------------------
bool MBExchange::pollRTU(std::shared_ptr<MBConfig::RTUDevice> &dev,
                         MBConfig::RegMap::iterator &it) {
  auto p = it->second;

  if (dev->mode == MBConfig::emSkipExchange) {
    mblog3 << myname << "(pollRTU): SKIP EXCHANGE (mode=emSkipExchange) "
           << " mbaddr=" << ModbusRTU::addr2str(dev->mbaddr) << endl;
    return false;
  }
  ...
  if (mblog->is_level3()) {
    mblog3 << myname << "(pollRTU): poll "
           << " mbaddr=" << ModbusRTU::addr2str(dev->mbaddr)
           << " mbreg=" << ModbusRTU::dat2str(p->mbreg)
           << " mbfunc=" << p->mbfunc << " q_count=" << p->q_count
           << " mb_initOK=" << p->mb_initOK << " sm_initOK=" << p->sm_initOK
           << " mbval=" << p->mbval << endl;

    if (p->q_count > mbconf->maxQueryCount /* ModbusRTU::MAXDATALEN */) {
      mblog3 << myname << "(pollRTU): count(" << p->q_count << ") > MAXDATALEN("
             << mbconf->maxQueryCount /* ModbusRTU::MAXDATALEN */
             << " ..ignore..." << endl;
    }
  }
  ...
  if (p->q_count == 0) {
    mbinfo << myname
           << "(pollRTU): q_count=0 for mbreg=" << ModbusRTU::dat2str(p->mbreg)
  ...
  switch (p->mbfunc) {
  case ModbusRTU::fnReadInputRegisters: {
    ModbusRTU::ReadInputRetMessage ret =
        mb->read04(dev->mbaddr, p->mbreg, p->q_count);
  ...
  case ModbusRTU::fnReadOutputRegisters: {
    ModbusRTU::ReadOutputRetMessage ret =
        mb->read03(dev->mbaddr, p->mbreg, p->q_count);
  ...
  case ModbusRTU::fnReadInputStatus: {
    ModbusRTU::ReadInputStatusRetMessage ret =
        mb->read02(dev->mbaddr, p->mbreg, p->q_count);
    size_t m = 0;

    for (uint i = 0; i < ret.bcnt; i++) {
      ModbusRTU::DataBits b(ret.data[i]);

      for (size_t k = 0; k < ModbusRTU::BitsPerByte && m < p->q_count;
           k++, it++, m++) {
        it->second->mbval = b[k];
        it->second->mb_initOK = true;
      }
  ...
  case ModbusRTU::fnReadCoilStatus: {
    ModbusRTU::ReadCoilRetMessage ret =
        mb->read01(dev->mbaddr, p->mbreg, p->q_count);
    size_t m = 0;
  ...
  case ModbusRTU::fnWriteOutputSingleRegister: {
    if (p->q_count != 1) {
      mbcrit << myname << "(pollRTU): mbreg=" << ModbusRTU::dat2str(p->mbreg)
             << " IGNORE WRITE SINGLE REGISTER (0x06) q_count=" << p->q_count
             << " ..." << endl;
      return false;
    }
    ...
    if (!p->sm_initOK) {
      mblog3 << myname << "(pollRTU): mbreg=" << ModbusRTU::dat2str(p->mbreg)
             << " slist=" << (*p) << " IGNORE...(sm_initOK=false)" << endl;
      return true;
    }
  ...
  case ModbusRTU::fnWriteOutputRegisters: {
    ...
    // после отправки скидываем флаг изменения значения первого регистра в
    // группе т.к. по нему проверяем условие для отправки.
    p->mbval_changed = false;
  } break;
  ...
  case ModbusRTU::fnForceSingleCoil: {
    if (p->q_count != 1) {
      mbcrit << myname << "(pollRTU): mbreg=" << ModbusRTU::dat2str(p->mbreg)
             << " IGNORE FORCE SINGLE COIL (0x05) q_count=" << p->q_count
             << " ..." << endl;
      return false;
    }
  ...
  for (auto it1 = mbconf->devices.begin(); it1 != mbconf->devices.end();
    ...
    if (d->mode_id != DefaultObjectId) {
      try {
        ...
        mblog3 << myname
               << "(updateSM): CORBA::SystemException: " << ex.NP_minorString()
               << endl;
      } catch (const uniset::Exception &ex) {
        mblog3 << myname << "(updateSM): " << ex << endl;
      } catch (std::exception &ex) {
        mblog3 << myname << "(updateSM): check modeSensor: " << ex.what()
               << endl;
      }
      ...
      if (d->safemode_id != DefaultObjectId) {
        try {
          if (!shm->isLocalwork()) {
            if (shm->localGetValue(d->safemode_it, d->safemode_id) ==
                d->safemode_value)
          ...
          mblog3 << myname << "(updateSM): check safemodeSensor: " << ex.what()
                 << endl;
        }
      }
    }
...
// ------------------------------------------------------------------------------------------
uint16_t MBExchange::forceSetBits(uint16_t value, uint16_t set, uint16_t mask,
                                  uint8_t offset) {
  if (mask == 0)
    return set;
...
// ------------------------------------------------------------------------------------------
uint16_t MBExchange::setBits(uint16_t value, uint16_t set, uint16_t mask,
                             uint8_t offset) {
  if (mask == 0)
    return value;
...
void MBExchange::updateRSProperty(MBConfig::RSProperty *p, bool write_only) {
  ...
  // если ещё не обменивались ни разу с устройством, то игнорируем (не обновляем
  // значение в SM)
  ...
  mblog3 << myname << "(updateP): sid=" << p->si.id << " mbval=" << r->mbval
         << " vtype=" << p->vType << " rnum=" << p->rnum
         << " nbit=" << (int)p->nbit << " mask=" << p->mask
         << " offset=" << (int)p->offset << " save=" << save
         << " ioype=" << p->stype << " mb_initOK=" << r->mb_initOK
         << " sm_initOK=" << r->sm_initOK << " safeMode=" << isSafeMode(r->dev)
         << " safevalDefined=" << p->safevalDefined << endl;
  ...
  try {
    if (p->vType == VTypes::vtUnknown) {
      ...
      if (p->nbit >= 0) {
        if (save) {
          if (r->mb_initOK) {
            bool set = useSafeval ? p->safeval
                                  : IOBase::processingAsDO(p, shm, force_out);
            b.set(p->nbit, set);
            r->setMBVal(b.mdata());
            r->sm_initOK = true;
          }
      ...
      if (p->rnum <= 1) {
        if (save) {
          if (r->mb_initOK) {
            ...
            r->setMBVal(
                MBExchange::forceSetBits(r->mbval, val, p->mask, p->offset));
            r->sm_initOK = true;
          }
        } else {
      ...
      if (p->nbyte <= 0 || p->nbyte > VTypes::Byte::bsize) {
        mbcrit << myname << "(updateRSProperty): IGNORE item: reg="
               << ModbusRTU::dat2str(r->mbreg) << " vtype=" << p->vType
               << " but nbyte=" << p->nbyte << endl;
        return;
      }
      ...
      if (save) {
        if (r->mb_initOK) {
          long v = useSafeval ? p->safeval
                              : IOBase::processingAsAO(p, shm, force_out);
          VTypes::Byte b(r->mbval);
          b.raw.b[p->nbyte - 1] = v;

          r->setMBVal(
              MBExchange::forceSetBits(r->mbval, b.raw.w, p->mask, p->offset));
          r->sm_initOK = true;
        }
      } else {
        if (useSafeval)
          ...
          VTypes::Byte b(r->mbval);
          uint16_t val =
              MBExchange::getBits(b.raw.b[p->nbyte - 1], p->mask, p->offset);
      ...
      if (save) {
        if (r->mb_initOK) {
          ...
          float f = useSafeval ? (float)p->safeval
                               : IOBase::processingFasAO(p, shm, force_out);
          bool changed = false;
  ...
  for (auto it = r->slst.begin(); it != r->slst.end(); ++it) {
    try {
      ...
      if (r->mtrType == MTR::mtT1) {
        if (save)
          r->mbval = safeMode ? it->safeval
                              : IOBase::processingAsAO(&(*it), shm, force_out);
        else {
          if (safeMode) {
      ...
      if (r->mtrType == MTR::mtT4) {
        if (save) {
          mbwarn << myname << "(updateMTR): write (T4) reg("
                 << dat2str(r->mbreg) << ") to MTR NOT YET!!!" << endl;
        } else {
          if (safeMode) {
            IOBase::processingAsAI(&(*it), it->safeval, shm, force);
          } else {
      ...
      if (r->mtrType == MTR::mtT16) {
        if (save) {
          ...
          float fval = safeMode
                           ? (float)it->safeval
                           : IOBase::processingFasAO(&(*it), shm, force_out);
      ...
      if (r->mtrType == MTR::mtF1) {
        ...
        if (save) {
          //! \warning НЕ РЕШЕНО safeval --> float !
          float f = safeMode ? (float)it->safeval
                             : IOBase::processingFasAO(&(*it), shm, force_out);
          MTR::F1 f1(f);
      ...
      mblog3 << myname
             << "(updateMTR): CORBA::SystemException: " << ex.NP_minorString()
             << endl;
    } catch (const uniset::Exception &ex) {
...
void MBExchange::updateRTU188(MBConfig::RegMap::iterator &rit) {
  ...
  if (isWriteFunction(r->mbfunc)) {
    mblog3 << myname << "(updateRTU188): write reg(" << dat2str(r->mbreg)
           << ") to RTU188 NOT YET!!!" << endl;
    return;
  }
  ...
  for (auto it = r->slst.begin(); it != r->slst.end(); ++it) {
    try {
      ...
      if (it->stype == UniversalIO::DI) {
        bool set = safeMode ? (bool)it->safeval
                            : r->dev->rtu188->getState(r->rtuJack, r->rtuChan,
                                                       it->stype);
        IOBase::processingAsDI(&(*it), set, shm, force);
      } else if (it->stype == UniversalIO::AI) {
        long val = safeMode ? it->safeval
                            : r->dev->rtu188->getInt(r->rtuJack, r->rtuChan,
                                                     it->stype);
...
void MBExchange::sysCommand(const uniset::SystemMessage *sm) {
  ...
  case SystemMessage::StartUp: {
    if (!logserv_host.empty() && logserv_port != 0 && !logserv->isRunning()) {
      mbinfo << myname << "(init): run log server " << logserv_host << ":"
             << logserv_port << endl;
    ...
    mbinfo << myname
           << "(sysCommand): device map size= " << mbconf->devices.size()
           << endl;
  ...
  case SystemMessage::WatchDog: {
    ...
    // при заказе датчиков, а если SM вылетит, то вместе с этим
    // процессом(MBExchange)
    if (shm->isLocalwork())
      break;
...
// ------------------------------------------------------------------------------------------
bool MBExchange::reconfigure(const std::shared_ptr<uniset::UniXML> &xml,
...
bool MBExchange::reload(const std::string &confile) {
  ...
  try {
    ...
    string conf_name(uniset_conf()->getArgParam(
        "--" + mbconf->prefix + "-confnode", myname));
    xmlNode *newCnode = xml->findNode(xml->getFirstNode(), conf_name);

    if (newCnode == NULL) {
      mbcrit << myname << "(reload): reload config from '" << confile
             << "' FAILED: not found conf node '" << conf_name << "'" << endl;
      return false;
    }
    ...
    if (newConf->devices.empty()) {
      mbcrit << myname << "(reload): reload config from '" << confile
             << "' FAILED: empty devices list" << std::endl;
      return false;
    }
...
void MBExchange::askSensors(UniversalIO::UIOCommand cmd) {
  if (!shm->waitSMworking(sidTestSMReady, activateTimeout, 50)) {
    ostringstream err;
    err << myname
        << "(askSensors): Не дождались готовности(work) SharedMemory к работе "
           "в течение "
  ...
  for (auto it1 = mbconf->devices.begin(); it1 != mbconf->devices.end();
    ...
    try {
      ...
      mbwarn << myname << "(askSensors): "
             << "("
             << " mode_id=" << d->mode_id << " safemode_id=" << d->safemode_id
             << ").. catch..." << std::endl;
    }
    ...
    for (auto &&m : d->pollmap) {
      ...
      for (auto it = regmap->begin(); it != regmap->end(); ++it) {
        ...
        for (auto i = it->second->slst.begin(); i != it->second->slst.end();
             ++i) {
          try {
            ...
            mbwarn << myname << "(askSensors): id=" << i->si.id << " catch..."
                   << std::endl;
          }
        }
      }
...
void MBExchange::sensorInfo(const uniset::SensorMessage *sm) {
  if (sm->id == sidExchangeMode) {
    ...
    mblog3 << myname << "(sensorInfo): exchange MODE=" << sm->value
           << std::endl;
    // return; // этот датчик может встречаться и в списке обмена.. поэтому
    // делать return нельзя.
  }
        ...
        for (auto &&i : it.second->slst) {
          if (sm->id == i.si.id && sm->node == i.si.node) {
            mbinfo << myname << "(sensorInfo): si.id=" << sm->id
                   ...
                   << " val=" << sm->value << " mb_initOK=" << i.reg->mb_initOK
                   << endl;
...
bool MBExchange::poll() {
  ...
  for (auto &&it1 : mbconf->devices) {
    ...
    for (auto &&m : d->pollmap) {
      if (m.first != MBConfig::changeOnlyWrite && m.first > 1 &&
          (ncycle % m.first) != 0)
        continue;
      ...
      for (auto it = regmap->begin(); it != regmap->end(); ++it) {
        ...
        // Для pollfactor == 65535 пропускаем для отсылки только регистры,
        ...
        try {
          ...
          if (mblog->debugging(Debug::LEVEL3)) {
            mblog3 << myname << "(poll): FAILED ask addr="
                   << ModbusRTU::addr2str(d->mbaddr)
                   << " reg=" << ModbusRTU::dat2str(it->second->mbreg)
                   << " for sensors: " << to_string(it->second->slst) << endl
                   << " err: " << ex << endl;
          }
  ...
  if (stat_time > 0 && ptStatistic.checkTime()) {
    ostringstream s;
    s << "number of calls is " << poll_count << " (poll time: " << stat_time
      << " sec)";
    statInfo = s.str();
  ...
  if (allNotRespond && exchangeMode != MBConfig::emSkipExchange &&
      ptReopen.checkTime()) {
    mbwarn << myname << ": REOPEN timeout..(" << ptReopen.getInterval() << ")"
           << endl;
  ...
  for (const auto &it1 : mbconf->devices) {
    ...
    if (d->checkRespond(mblog) && d->resp_id != DefaultObjectId) {
      try {
        ...
        mblog4 << myname << ": SAVE NEW " << (d->resp_invert ? "NOT" : "")
               << " respond state=" << set
               << " for addr=" << ModbusRTU::addr2str(d->mbaddr)
               << " respond_id=" << d->resp_id << " state=" << d->resp_state
               << " [ invert=" << d->resp_invert
               << " timeout=" << d->resp_Delay.getOffDelay()
               << " numreply=" << d->numreply
               << " prev_numreply=" << d->prev_numreply << " ]" << endl;
...
MBExchange::httpHelp(const Poco::URI::QueryParameters &p) {
...
// ----------------------------------------------------------------------------
Poco::JSON::Object::Ptr
MBExchange::httpRequest(const string &req,

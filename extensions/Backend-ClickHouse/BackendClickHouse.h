...
\code
        <BackendClickHouse name="BackendClickHouse1" dbhost="localhost"
dbport="9000" dbuser="" dbpass="" filter_field="clickhouse" filter_value="1"
...
\endcode
    ...
    - \b dbuser - user
    - \b dbpass - password
    - \b dbtablename - Имя таблицы в которую писать данные. По умолчанию
main_history.
    - \b filter_field - поле у датчика, определяющее, что его нужно сохранять в
БД
    - \b filter_value - значение \b filter_field, определяющее, что датчик нужно
сохранять в БД
    - \b tags - теги которые будут добавлены для каждой записи (перечисляются
через пробел)
...
\code
    <sensors>
        ...
        <item id="54" iotype="AI" name="AI54_S" textname="AI sensor 54"
clickhouse="1" clickhouse_tags=""/> <item id="55" iotype="AI" name="AI55_S"
textname="AI sensor 55" clickhouse="1" clickhouse_tags="tag1=val1 tag2=val2"/>
        ...
    </sensors>
\endcode

\section sec_ClickHouse_Queue Буфер на запись в БД
В данной реализации встроен специальный буфер, который накапливает данные и
скидывает их пачкой в БД. Так же он является защитным механизмом на случай если
БД временно недоступна. Параметры буфера задаются аргументами командной строки
или в конфигурационном файле. Доступны следующие параметры:
- \b bufSize - размер буфера, при заполнении которого происходит запись данных в
БД
- \b bufMaxSize - максимальный размер буфера, при котором все данные теряются
(буфер чиститься)
...
- \b sizeOfMessageQueue - Размер очереди сообщений для обработки изменений по
датчикам. При большом количестве отслеживаемых датчиков, размер должен быть
достаточным, чтобы не терять изменения.
...
\section sec_ClickHouse_Admin Впомогательная утилита uniset2-clickhouse-admin
 В состав проекта так же входит утилита uniset2-clickhouse-admin облегчающая
взаимодействие с ClickHouse. В частности утилита позволяет сгенерировать
csv-файлы словарей для подключения к проекту.
 ...
 \code
  ...
  Valid command are:
    -h, --help            - display help
    -d, --dicts           - Generate dictionaries data (csv files) for
Clickhouse -c, --create [dbname] - Create database. Default dbname=uniset -s,
...
ClickHouse. В результате в БД будут доступны словари `dict_sensors`,
`dict_objects`, `dict_nodes` позволяющие использовать имена датчиков и другие
характеристики в запросах. Пример простого запроса выводящего историю изменения
датчиков за последний час \code SELECT timestamp, value,
 ...
 SELECT
     ...
     dictGetString('dict_nodes', 'ip', node_hid)
 FROM main_history
 WHERE timestamp > now() - toIntervalHour(30) and name_hid =
cityHash64('Sensor1_AS') \endcode
 ...
 Описание доступных в словарях полей можно посмотреть в файле
`clickhouse_uniset_dictionary.xml`
*/
//------------------------------------------------------------------------------------------
/*! Реализация DBServer для Clickhouse */
class BackendClickHouse : public UObject_SK {
public:
  BackendClickHouse(uniset::ObjectId objId, xmlNode *cnode,
  ...
  static std::shared_ptr<BackendClickHouse>
  init_clickhouse(int argc, const char *const *argv, uniset::ObjectId shmID,
                  const std::shared_ptr<SharedMemory> &ic = nullptr,
                  const std::string &prefix = "clickhouse");
